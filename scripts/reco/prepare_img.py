...
def crop_and_save(detection, img, save_path, img_name):
    ...
    for i, det in enumerate(detection):
        ...
        for j in len(face_augmented):
            cv2.imwrite(os.path.join(save_path, f"{img_name}_{i}_{j}_AUGMENTED.png"), face_augmented)
...
def prepare_img(img_path):
    ...
    log_to_file(f"Getting face for {img_path}...", "Ingo")
