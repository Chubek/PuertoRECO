...
def validate_env():
    ...
    if 'DB_PATH' in temp:
        if temp['DB_PATH'] == "" or not os.path.isdir(temp['DB_PATH']):    
            print("Env file configured incorrectly: DB_PATH not set or is not a directory.")
    ...
    if 'MODEL_PATH' in temp:
        if temp['MODEL_PATH'] == "" or not os.path.isdir(temp['MODEL_PATH']):
            print("Env file configured incorrectly: MODEL_PATH not set or is not a directory.")
        ...
        print("MODEL_PATH is not in .env")
    ...
    if 'SELECTED_MODELS' in temp:
        if temp['SELECTED_MODELS'] == "" or not re.match(regex_models, temp['SELECTED_MODELS']):
            print("Env file configured incorrectly: SELECTED_MODELS is either empty, or doesn't match the necessary pattern.")
        ...
        print("SELECTED_MODELS is not in .env")
    ...
    if 'MONGO_DB' in temp:
        if temp['MONGO_DB'] == '' or not re.match(r"[^\W\d_]+", temp['MONGO_DB']):
            print("Env file configured incorrectly: MONGO_DB is either empty or does not match pattern '[^\\W\\d_]+'")
    ...
    if 'MONGO_COLL' in temp:    
        if temp['MONGO_COLL'] == '' or not re.match(r"[^\W\d_]+", temp['MONGO_COLL']):
            print("Env file configured incorrectly: MONGO_COLL is either empty or does not match pattern '[^\\W\\d_]+'")
        ...
        print("MONGO_COLL is not in .env")
    ...
    if 'MODEL_URL' in temp:
        if temp['MODEL_URL'] == '' or not re.match(regex_url, temp['MODEL_URL']):
            print("Env file configured incorrectly: MODEL_URL is either empty or is not a valid URL.")
    ...
    if 'TARGET_WIDTH' in temp:
        if temp['TARGET_WIDTH'] == '' or not temp['TARGET_WIDTH'].isnumeric():
            print("Env file configured incorrectly: TARGET_WIDTH is either empty or not numeric.")
        ...
        print("TARGET_WIDTH is not in .env")
    ...
    if 'TARGET_HEIGHT' in temp:        
        if temp['TARGET_HEIGHT'] == '' or not temp['TARGET_HEIGHT'].isnumeric():
            print("Env file configured incorrectly: TARGET_HEIGHT is either empty or not numeric.")
        ...
        print("TARGET_HEIGHT is not in .env")
    ...
    if 'NUM_AUG' in temp:
        if temp['NUM_AUG'] == '' or not temp['NUM_AUG'].isnumeric():
            print("Env file configured incorrectly: NUM_AUG is either empty or not numeric.")
    ...
    if 'SIM_FUNC' in temp:
        if temp['SIM_FUNC'] == '' or not temp['SIM_FUNC'] in ["cosine", "euclidean", "euclidean_l2"]:
            print("Env file configured incorrectly: SIM_FUNC is either empty or not approved. Must be in ['cosine', 'euclidean', 'euclidean_l2'].")
    ...
    if 'LOG_LOC' in temp:
        ...
        if os.path.exists(temp['LOG_LOC']):
            if is_binary(temp['LOG_LOC']):
                print("Env file configured incorrectly: LOG_LOC is binary.")
    ...
    if 'ID_REGEX' in temp:
        if temp['ID_REGEX'] == '':
            print("Env file configured incorrectly: ID_REGEX is empty.")
        ...
        try:
            ...
            print("Env file configured incorrectly: ID_REGEX is not a valid pattern.")
    ...
    if 'VER_TOL_AUG' in temp:
        if temp['VER_TOL_AUG'] == '' or not temp['VER_TOL_AUG'].isnumeric():
            print("Env file configured incorrectly: VER_TOL_AUG is empty or is not numeric.")
        ...
        print("VER_TOL_AUG is not in .env")
        log_to_file("VER_TOL_AUG is not in .env", "ERROR")
        not_in_env.append("VER_TOL_AUG")

    print(f".env file validated. {len(not_in_env)} keys not found, {len(env_errs)} errors found.")
